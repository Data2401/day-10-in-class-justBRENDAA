...
```{r}
colnames(USPE)
# error caused by taking Category as a column 

# original pivot 
# pivot_longer(
#   data = USPE,
#   cols = colnames(USPE),    <- this caused my error 
#   names_to = "Year",
#   values_to = "Val"
# )
...
USPE2 = pivot_longer(
  data = USPE,
  cols = colnames(USPE)[2:length(cols)],
  names_to = "Year",
  values_to = "Spending"
)
...
```{r}
USPE2$Spending[USPE2$Category == 'Personal Care' & USPE2$Year == 'X1940']
...
```{r}
USPE2$Spending[USPE2$Category == 'Food and Tobacco' & USPE2$Year == 'X1960']
...
```{r}
USPE2$Category[USPE2$Spending == max(USPE2$Spending[USPE2$Year == 'X1960'])]
...
lowest_category <- function(year){
  ...
  USPE2$Category[USPE2$Spending == min(USPE2$Spending[USPE2$Year == year])]
}
```
